...
across synthetic datasets produced by CTGAN or TVAE models.

It reads all CSV files from the specified input folders, merges them by label, 
and visualizes the feature distributions per attack class (e.g., BENIGN, DoS Hulk, FTP-BruteForce).

Each generated plot shows the distribution of a single feature across all labels,
helping visualize how synthetic data samples vary between classes.

The output consists of one PNG file per feature, stored in the specified output directory.

Use this tool to:
- Compare feature distributions between different synthetic generation methods (CTGAN vs TVAE)
- Validate the quality and diversity of generated synthetic data
- Observe overlaps or separations between attack types in feature space


Example Configuration TVAE:
