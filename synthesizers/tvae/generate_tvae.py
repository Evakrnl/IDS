...
This script trains or loads per-label TVAE models, generates synthetic samples,
...
3) For each selected label, optionally retrain a TVAE model and choose sample count and epochs.
...
def choose_tvae_hyperparams(n_rows: int) -> Tuple[int, Tuple[int, ...], Tuple[int, ...], int]:
    """
    Choose TVAE hyperparameters based on dataset size.
    ...
    -------
    tuple
        A tuple of (batch_size, compress_dims, decompress_dims, embedding_dim).
    """

    if n_rows < 100:
        bs, comp, decomp, emb = min(4, n_rows), (16,), (16,), 16
    elif n_rows < 1_000:
        bs, comp, decomp, emb = 128, (64,), (64,), 32
    elif n_rows < 10_000:
        bs, comp, decomp, emb = 256, (128, 64), (64, 128), 64
    elif n_rows < 100_000:
        bs, comp, decomp, emb = 512, (256, 128), (128, 256), 128
    elif n_rows < 1_000_000:
        bs, comp, decomp, emb = 1024, (512, 256, 128), (128, 256, 512), 128
    else:
        bs, comp, decomp, emb = 2048, (1024, 512, 256), (256, 512, 1024), 128

    return bs, comp, decomp, emb
...
def main() -> None:
    """
    Entry point for the interactive TVAE generation and evaluation workflow.
    ...
    for label in selected_labels:
        ...
        if train_new_model:
            ...
            batch_size, comp_dims, decomp_dims, embedding_dim= choose_tvae_hyperparams(len(df_data))
